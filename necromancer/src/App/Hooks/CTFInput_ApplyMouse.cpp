...
static void* GetPlayerShared(C_TFPlayer* pPlayer)
{
	static const auto dwShared = NetVars::GetNetVar("CTFPlayer", "m_Shared");
	return reinterpret_cast<void*>(reinterpret_cast<std::uintptr_t>(pPlayer) + dwShared);
}

MAKE_HOOK(CTFInput_ApplyMouse, Signatures::CTFInput_ApplyMouse(), void, __fastcall,
	void* rcx, Vec3& viewangles, CUserCmd* cmd, float mouse_x, float mouse_y)
{
	CALL_ORIGINAL(rcx, viewangles, cmd, mouse_x, mouse_y);

	if (!CFG::Misc_Shield_Turn_Rate)
		return;
	...
	void* pShared = GetPlayerShared(pLocal);
	if (!pShared)
		return;

	float flOriginalFrame = I::GlobalVars->frametime;
	I::GlobalVars->frametime = TICK_INTERVAL;
	float flCap = Signatures::CTFPlayerShared_CalculateChargeCap.Call<float>(pShared) * 2.5f;
...
MAKE_HOOK(CTFInput_CAM_CapYaw, Signatures::CTFInput_CAM_CapYaw(), float, __fastcall,
	void* rcx, float fVal)
{
	if (!CFG::Misc_Shield_Turn_Rate)
