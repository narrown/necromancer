...
void CAmalgamMovementSimulation::Store(MoveStorage& tStorage)
{
...
void CAmalgamMovementSimulation::Reset(MoveStorage& tStorage)
{
	if (tStorage.m_pData)
	{
		// Safety check - make sure player is still valid
		if (!tStorage.m_pPlayer || !IsPlayer(tStorage.m_pPlayer))
		{
...
void CAmalgamMovementSimulation::Clear()
{
	m_mRecords.clear();
	m_mSimTimes.clear();
}

void CAmalgamMovementSimulation::Store()
{
	for (auto pEntity : g_AmalgamEntitiesExt.GetGroup(EntityEnum::PlayerAll))
	{
		...
		if (!IsAlive(pPlayer) || IsAGhost(pPlayer) || pPlayer->m_vecVelocity().IsZero())
		{
			vRecords.clear();
			continue;
		}
		else if (!g_AmalgamEntitiesExt.GetDeltaTime(pPlayer->entindex()))
			continue;
		...
		MoveData_Record* pLastRecord = !vRecords.empty() ? &vRecords.front() : nullptr;
		vRecords.emplace_front(
			vDirection,
			pPlayer->m_flSimulationTime(),
			IsSwimming(pPlayer) ? 2 : IsOnGround(pPlayer) ? 0 : 1,
			vVelocity,
			vOrigin
		);
		MoveData_Record& tCurRecord = vRecords.front();
		if (vRecords.size() > 66)
			vRecords.pop_back();
			...
			*/
			{
				...
				SDK::TraceHull(pLastRecord->m_vOrigin, pLastRecord->m_vOrigin + pLastRecord->m_vVelocity * TICK_INTERVAL, pPlayer->m_vecMins() + 0.125f, pPlayer->m_vecMaxs() - 0.125f, SolidMask(pPlayer), &filter, &trace);
	...
	for (auto pEntity : g_AmalgamEntitiesExt.GetGroup(EntityEnum::PlayerAll))
	{
		...
		if (iIndex == I::EngineClient->GetLocalPlayer() || !IsAlive(pPlayer) || IsAGhost(pPlayer))
		{
			vSimTimes.clear();
			continue;
		}

		float flDeltaTime = g_AmalgamEntitiesExt.GetDeltaTime(iIndex);
		if (!flDeltaTime)
			continue;
...
bool CAmalgamMovementSimulation::Initialize(C_BaseEntity* pEntity, MoveStorage& tStorage, bool bHitchance, bool bStrafe)
{
	if (!pEntity || !IsPlayer(pEntity) || !IsAlive(pEntity->As<CTFPlayer>()))
	{
	...
	if (auto pAvgVelocity = g_AmalgamEntitiesExt.GetAvgVelocity(pPlayer->entindex()))
		pPlayer->m_vecVelocity() = *pAvgVelocity; // only use average velocity here

	if (pPlayer->m_bDucked() = IsDucking(pPlayer))
	{
	...
	if (pPlayer != g_AmalgamEntitiesExt.GetLocal())
	{
	...
	for (int i = 0; i < g_AmalgamEntitiesExt.GetChoke(pPlayer->entindex()); i++)
		RunTick(tStorage);

	return true;
}

bool CAmalgamMovementSimulation::SetupMoveData(MoveStorage& tStorage)
{
	...
	if (!tStorage.m_MoveData.m_vecVelocity.To2D().IsZero())
	{
		...
		else
		{
			if (!tStorage.m_pPlayer->InCond(TF_COND_SHIELD_CHARGE))
				...
				tStorage.m_MoveData.m_vecViewAngles = g_AmalgamEntitiesExt.GetEyeAngles(iIndex);
		}
	...
	tStorage.m_bDirectMove = IsOnGround(tStorage.m_pPlayer) || IsSwimming(tStorage.m_pPlayer);
...
static inline void VisualizeRecords(MoveData_Record& tRecord1, MoveData_Record& tRecord2, Color_t tColor, float flStraightFuzzyValue)
{
...
static inline bool GetYawDifference(MoveData_Record& tRecord1, MoveData_Record& tRecord2, bool bStart, float* pYaw, float flStraightFuzzyValue, int iMaxChanges = 0, int iMaxChangeTime = 0, float flMaxSpeed = 0.f)
{
	const float flYaw1 = Math::VectorAngles(tRecord1.m_vDirection).y, flYaw2 = Math::VectorAngles(tRecord2.m_vDirection).y;
	const float flTime1 = tRecord1.m_flSimTime, flTime2 = tRecord2.m_flSimTime;
	const int iTicks = std::max(TIME_TO_TICKS(flTime1 - flTime2), 1);
...
void CAmalgamMovementSimulation::GetAverageYaw(MoveStorage& tStorage, int iSamples)
{
	...
	*/
	...
	if (pPlayer->entindex() != I::EngineClient->GetLocalPlayer())
	{
		...
		if (auto pLocal = g_AmalgamEntitiesExt.GetLocal())
...
bool CAmalgamMovementSimulation::StrafePrediction(MoveStorage& tStorage, int iSamples)
{
...
bool CAmalgamMovementSimulation::SetDuck(MoveStorage& tStorage, bool bDuck) // this only touches origin, bounds
{
	...
	if (!IsOnGround(tStorage.m_pPlayer))
	{
		...
		else
		{
			...
			CGameTrace trace = {};
			CTraceFilterWorldAndPropsOnlyAmalgam filter = {};
			SDK::TraceHull(vOrigin, vOrigin, vHullMins, vHullMaxs, SolidMask(tStorage.m_pPlayer), &filter, &trace);
			if (trace.DidHit())
				return false;
...
void CAmalgamMovementSimulation::SetBounds(C_TFPlayer* pPlayer)
{
	if (pPlayer->entindex() == I::EngineClient->GetLocalPlayer())
		return;
...
void CAmalgamMovementSimulation::RestoreBounds(C_TFPlayer* pPlayer)
{
	if (pPlayer->entindex() == I::EngineClient->GetLocalPlayer())
		return;
...
void CAmalgamMovementSimulation::RunTick(MoveStorage& tStorage, bool bPath, std::function<void(CMoveData&)>* pCallback)
{
	if (tStorage.m_bFailed || !tStorage.m_pPlayer || !IsPlayer(tStorage.m_pPlayer))
		return;

	// Limit path size to prevent memory issues (max ~5 seconds of simulation)
	if (bPath && tStorage.m_vPath.size() < 333)
	...
	if (tStorage.m_pPlayer->m_bDucked() && IsOnGround(tStorage.m_pPlayer) && !IsSwimming(tStorage.m_pPlayer))
		tStorage.m_MoveData.m_flClientMaxSpeed /= 3;

	if (tStorage.m_bBunnyHop && IsOnGround(tStorage.m_pPlayer) && !tStorage.m_pPlayer->m_bDucked())
	{
...
void CAmalgamMovementSimulation::RunTick(MoveStorage& tStorage, bool bPath, std::function<void(CMoveData&)> fCallback)
{
	RunTick(tStorage, bPath, &fCallback);
}

void CAmalgamMovementSimulation::Restore(MoveStorage& tStorage)
{
	if (tStorage.m_bInitFailed || !tStorage.m_pPlayer)
		return;

	// Safety check - make sure player is still valid before accessing
	if (!IsPlayer(tStorage.m_pPlayer))
	{
		// Just reset the data without touching the player
		if (tStorage.m_pData)
		{
			I::MemAlloc->Free(tStorage.m_pData);
			tStorage.m_pData = nullptr;
...
float CAmalgamMovementSimulation::GetPredictedDelta(C_BaseEntity* pEntity)
{
