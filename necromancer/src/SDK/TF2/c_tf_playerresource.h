...
public:
	...
	// Helper functions to access array netvars by index (like Amalgam's NETVAR_ARRAY)
	float GetNextRespawnTime(int playerindex)
	{
		static int nOffset = NetVars::GetNetVar("CTFPlayerResource", "m_flNextRespawnTime");
		if (nOffset == 0) return 0.f;
		return *reinterpret_cast<float*>((reinterpret_cast<std::uintptr_t>(this) + nOffset) + (playerindex * sizeof(float)));
	}

	int GetPlayerClass(int playerindex)
	{
		static int nOffset = NetVars::GetNetVar("CTFPlayerResource", "m_iPlayerClass");
