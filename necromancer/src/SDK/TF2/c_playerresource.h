...
public:
	...
	int GetPing(int playerindex)
	{
		...
		return *reinterpret_cast<int *>((reinterpret_cast<std::uintptr_t>(this) + nOffset) + (playerindex * sizeof(int)));
	}

	int GetTeam(int playerindex)
	{
		static int nOffset = NetVars::GetNetVar("CPlayerResource", "m_iTeam");
		if (nOffset == 0) return 0;
		return *reinterpret_cast<int *>((reinterpret_cast<std::uintptr_t>(this) + nOffset) + (playerindex * sizeof(int)));
	}

	bool IsValid(int playerindex)
	{
		static int nOffset = NetVars::GetNetVar("CPlayerResource", "m_bValid");
		if (nOffset == 0) return false;
		return *reinterpret_cast<bool *>((reinterpret_cast<std::uintptr_t>(this) + nOffset) + (playerindex * sizeof(bool)));
	}

	bool IsAlive(int playerindex)
	{
		static int nOffset = NetVars::GetNetVar("CPlayerResource", "m_bAlive");
		if (nOffset == 0) return false;
		return *reinterpret_cast<bool *>((reinterpret_cast<std::uintptr_t>(this) + nOffset) + (playerindex * sizeof(bool)));
	}

	// Check if player is a fake/bot player (like Amalgam's IsFakePlayer)
	bool IsFakePlayer(int playerindex)
	{
		// If ping is 0, it's likely a bot
